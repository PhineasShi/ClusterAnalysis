...
public:
	Cluster();
	Cluster( vector<vector<double> > &vecs, DistType distType = euclidean, double ratio = 0.5, int m = 2,double thr=0) :vecs(vecs), distType(distType), ratio(ratio), m(m), thr(thr) {};
	~Cluster();
